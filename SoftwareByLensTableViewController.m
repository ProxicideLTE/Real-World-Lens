//
//  SoftwareByLensTableViewController.m
//  LensLookupTable
//
...
@implementation SoftwareByLensTableViewController

@synthesize lensKitName;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *CellIdentifier = @"Software";
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LensFOVTableViewController* table = [LensFOVTableViewController alloc];
    ...
    table.softwareName = [listOfItems objectAtIndex:indexPath.row];
    table.previousView = 1;
        ...
        while (true) {
            ...
            switch (xmlTextReaderNodeType(reader)) {
                ...
                case XML_READER_TYPE_TEXT:
                    ...
                    // Determine if the software has been found.
                    if ([currentTagName isEqualToString:@"name"] && [currentTagValue isEqualToString:lensKitName]) {
                        foundLensKit = YES;
                        break;
                    }
                    
                    // If the lens kit is found and the the current tag is the supported software.
                    else if (foundLensKit && [currentTagName isEqualToString:@"supported_software"]) {
                        // Add object to the list for the view.
                        [listOfItems addObject:currentTagValue];
                    }
                    
                    // If if the current tag is the end tag stop populating the array.
                    else if (foundLensKit && [currentTagName isEqualToString:@"end"]) {
                        foundLensKit = NO;
                        break;
                    }
...
- (IBAction) backBtnUserClick:(id)sender {
    ...
    // View to go back to.
    LensKitView* table = [LensKitView alloc];
    AppNavigationController* view = [[AppNavigationController alloc] initWithRootViewController:table];
